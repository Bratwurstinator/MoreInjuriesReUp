...
namespace Oof
{
	...
	public class DOCPR : JobDriver
	{
		...
		{
			if(!hurtdude.health.hediffSet.hediffs.Any(O => O.def == Caula_DefOf.ChockingOnBlood))
			{
		...
		protected override IEnumerable<Toil> MakeNewToils()
		{
			...
			Hediff heffid = Patient.health.hediffSet.GetFirstHediffOfDef(Caula_DefOf.ChockingOnBlood);
			toil2.AddFinishAction(delegate
			{
				...
				//else if(heffid.def == Caula_DefOf.ChockingOnBlood)
				heffid.Severity -= ((Doctor.skills.skills.Find(o => o.def == SkillDefOf.Medicine).Level * 1.35f)) / 100;
			});
			Func<JobCondition> func = delegate
			{
				if(Patient.health.hediffSet.hediffs.Any(kurwa => kurwa.def == Caula_DefOf.ChockingOnBlood | kurwa.def.label == "heart attack"))
				{
					//////log.Message("a:v");
					return JobCondition.Ongoing;
				}
