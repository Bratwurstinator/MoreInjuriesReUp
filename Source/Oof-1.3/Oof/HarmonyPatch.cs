...
namespace Oof
{
    ...
    public class OofMod : Mod
    {
        ...
        public override void DoSettingsWindowContents(Rect inRect)
        {
            ...
            listingStandard.CheckboxLabeled("Toggle  chocking on blood mechanics", ref settings.chocking, "Toggle  chocking on blood mechanics");

            listingStandard.Label("Base chance of armor creating spall " +
                    "(at 1, the chance of creating spall is 0 with armor having 100% hp, 0.01 with armor 99% hp etc.) " + Math.Round(settings.MinSpallHealth, 2), 2);
                settings.MinSpallHealth = listingStandard.Slider(settings.MinSpallHealth, 0.1f, 1f);
           
            listingStandard.CheckboxLabeled("Toggle  chocking sounds", ref settings.somesound, "Toggle  chocking sounds");
            ...
            listingStandard.Label("Manage hypoxia chance (" + settings.hypoxiaChance.ToStringPercent() + ") and interval (" + (settings.hypoxiaInterval / 60) + "s)");

            settings.hypoxiaChance = (float)Math.Round(listingStandard.Slider(settings.hypoxiaChance, 0f, 1f), 2);

            settings.hypoxiaInterval = (int)listingStandard.Slider(settings.hypoxiaInterval, 300, 9000);
    ...
    public class ToggleSettings : ModSettings
    {
        ...
        public float hypoxiaChance = 0.2f;
        public bool anyColoration = true;
        public bool fireInhalation = true;
        public int hypoxiaInterval = 900;
        ...
        public override void ExposeData()
        {
            ...
            Scribe_Values.Look(ref chocking, "chokebool");
            ...
            Scribe_Values.Look(ref hypoxiaInterval, "hypoxiainterval");
    ...
    public class InjuriesComp : ThingComp
    {
        ...
        public override IEnumerable<FloatMenuOption> CompFloatMenuOptions(Pawn selPawn)
        {
            ...
            if (selPawn != papa)
            {
                if (papa.health.hediffSet.hediffs.Any(o => o.def.label == "heart attack" | o.def == Caula_DefOf.ChockingOnBlood))
                {
                    yield return new FloatMenuOption("Do cpr", delegate
                    {
        ...
        public void PuttingOnTourniqet()
        {
            ...
            {
                TendableInjuries = myself.health.hediffSet.GetInjuriesTendable() as List<Hediff_Injury>;
            }
        ...
        public void nreTEst(List<BodyPartRecord> dd)
        {
            ...
            {
                if (myself.health.hediffSet.hediffs.FindAll(o => o.Part != null && o.Part == bodp).Any(K => K.Bleeding))
                {
                    Hediff brun = HediffMaker.MakeHediff(DefDatabase<HediffDef>.AllDefs.ToList().Find(gg => gg != null && gg.defName == "StomachAcidBurn"), myself, bodp);
                    if (Rand.Chance(0.45f))
                    {
                        brun.Severity = Rand.Range(1, 7f);
                        myself.health.AddHediff(brun);
                    }
                }
        ...
        {
            Pawn myself = (Pawn)this.parent;
            return myself.health.hediffSet.GetNotMissingParts().ToList().FindAll(x => (x.def?.defName ?? "null") == "smolinstestine" | (x.def?.defName ?? "null") == "largeinstestine"
        ...
        public void InstestinalSpill(DamageWorker.DamageResult damage)
        {
            ...
            if (damage.parts?.Any(x => x.def?.defName == "smolinstestine" | x.def?.defName == "largeinstestine" | x.def?.defName == "Stomach") ?? false)
            {
                ...
                //dd.Add(myself.health.hediffSet.getpart().ToList().FindAll(x => x.def.defName == "smolinstestine" | x.def.defName == "largeinstestine" | x.def.defName == "Stomach"))
                nreTEst(dd);
        ...
        public override void PostPostApplyDamage(DamageInfo dinfo, float totalDamageDealt)
        {
                ...
                foreach (Hediff_Injury injury in papa.health.hediffSet.GetInjuriesTendable())
                {
                    ...
                    if (injury.Part.def.tags.Contains(BodyPartTagDefOf.BreathingSource) | injury.Part.def.tags.Contains(BodyPartTagDefOf.BreathingPathway) && injury.Bleeding && injury.BleedRate >= 0.20f)
                    {
                        if (Rand.Chance(0.70f))
                        {
                            ...
                            papa.health.hediffSet.GetFirstHediffOfDef(Caula_DefOf.ChockingOnBlood).TryGetComp<chockingcomp>().source = injury;
            ...
            if (OofMod.settings.EMPdisablesBionics)
            {
                ...
                {
                    var statdef = DefDatabase<StatDef>.AllDefs.Where(x => x.defName == "ArmorRating_Electric").FirstOrFallback();

                    float electricArmor = dad.GetStatValueForPawn(statdef, dad);

                    if ((dad?.apparel?.WornApparel ?? null) != null)
                    {
                        foreach (var def in dad.apparel.WornApparel)
                        {
                            electricArmor += def.GetStatValue(statdef);
                        }
                    }

                    if (electricArmor < 0.5f)
                    {
        ...
        public void FractureMeth(DamageWorker.DamageResult damage)
        {
            if (Settings.toggleFractures)
            {
                ...
                {
                    Pawn myself = (Pawn)this.parent;
                    if (damage?.parts?.Any(x => x.def.IsSolid(x, myself.health.hediffSet.hediffs) && !x.def.IsSkinCovered(x, myself.health.hediffSet) && x.def.bleedRate == 0) ?? false)
                    {
                        var partsbony = damage.parts.FindAll(x => x.def.IsSolid(x, myself.health.hediffSet.hediffs) && !x.def.IsSkinCovered(x, myself.health.hediffSet) && x.def.bleedRate == 0);

                        foreach (BodyPartRecord bone in partsbony)
                        {
                            Hediff fracture = HediffMaker.MakeHediff(DefDatabase<HediffDef>.AllDefs.ToList().Find(x => x.defName == "Fracture"), myself, bone);
                            myself.health.AddHediff(fracture);
                            if (OofMod.settings.smolBoniShits)
                            {
                                foreach (BodyPartRecord part in bone.parent.GetDirectChildParts())
                                {
                                    ...
                                    {
                                        Hediff shards = HediffMaker.MakeHediff(DefDatabase<HediffDef>.AllDefs.ToList().Find(x => x.defName == "BoneCut"), myself, part);
                                        shards.Severity = Rand.Range(1f, 5f);
                                        myself.health.AddHediff(shards);
                                    ...
                                    foreach (BodyPartRecord part2 in part.GetDirectChildParts())
                                    {
                                        if (Rand.Chance(0.10f))
                                        {
                                            Hediff shards = HediffMaker.MakeHediff(DefDatabase<HediffDef>.AllDefs.ToList().Find(x => x.defName == "BoneCut"), myself, part2);
                                            shards.Severity = Rand.Range(1f, 5f);
                                            myself.health.AddHediff(shards);
        ...
        public void PostDamageFull(DamageWorker.DamageResult damage)
        {
            ...
            if (damage != null)
            {
                if (damage.parts != null)
                {
                    ...
                    {
                        Hediff hedf = HediffMaker.MakeHediff(Caula_DefOf.SpinarCordPapasyliz, myself, damage.parts.Find(pp => pp.def == Caula_DefOf.SPinarCord));

                        myself.health.AddHediff(hedf, damage.parts.Find(pp => pp.def == Caula_DefOf.SPinarCord));
                    }
                }
            }
            ...
            if (Settings.lungcollapse)
            {
                if ( (damage?.parts?.Any(pp => pp.def.defName == "Lung") ?? false) && ((damage?.totalDamageDealt ?? 5) >= 9))
                {
                    if((damDef?.hediff?.injuryProps?.bleedRate ?? 0f) > 0f)
                    {
                        ...
                        Hediff hediff = HediffMaker.MakeHediff(Caula_DefOf.LungCollapse, myself, recorder);
                        myself.health.AddHediff(hediff);
                    }
            ...
            Hediff lol = HediffMaker.MakeHediff(HediffDefOf.Stab, myself) as Hediff_Injury;
