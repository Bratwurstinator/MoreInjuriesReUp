...
namespace Oof
{
	...
	public class A_JobDefOf : DefOf
	{
		...
		public static HediffDef ChockingOnTourni;
		...
		public override IEnumerable<Gizmo> CompGetGizmosExtra()
		{
			...
			if (dad.Downed)
			{
				if (dad.health.capacities.GetLevel(PawnCapacityDefOf.Manipulation) > 0.19f)
				{
					if (dad.CellsAdjacent8WayAndInside().Any(ttt => ttt.GetFirstThing(dad.Map, A_JobDefOf.tourniquet) != null) | dad.inventory.innerContainer.Any(x => x.def == A_JobDefOf.tourniquet))
					{
						...
						{
							defaultLabel = "Put on a tourniquet",
							defaultDesc = "While downed, the pawn still can grip things well enough to at least try to help themselves",
							icon = ContentFinder<Texture2D>.Get("Thing/tour", true),
							action = delegate
							{
								...
								foreach (BodyPartRecord bodypart in Limbs)
								{
									...
									if (true)
									{
										...
										options.Add(new FloatMenuOption("Put a tourniquet on " + bodypart.Label.Colorize(Color.green), delegate
										{
			...
			else
			{
				if (dad.inventory.innerContainer.Any(x => x.def == A_JobDefOf.tourniquet))
				{
					...
					{
						defaultLabel = "Put on a tourniquet",
						defaultDesc = "Put a tourniquet on yourself",
						icon = ContentFinder<Texture2D>.Get("Thing/tour", true),
						action = delegate
						{
							List<FloatMenuOption> options = new List<FloatMenuOption>();
			...
			if (test)
			{
				...
				if (selPawn.inventory.innerContainer.Any(F => F.def.defName == "tourniquet"))
				{
					foreach (BodyPartRecord bodypart in Limbs)
					{
						...
						if (true)
						{
							yield return new FloatMenuOption("Put a tourniquet on " + bodypart.Label.Colorize(Color.green), delegate
							{
								//missingPart = (Hediff_MissingPart)parent.health.hediffSet.hediffs.Find(x => x.def.hediffClass == typeof(Hediff_MissingPart) && (!x.Bleeding | !x.IsTended()));
	...
	public class TourniHediffComp : HediffComp
	{
		...
		public override void CompPostMake()
		{
			...
			{
				Hediff hed = HediffMaker.MakeHediff(A_JobDefOf.ChockingOnTourni, this.parent.pawn);
